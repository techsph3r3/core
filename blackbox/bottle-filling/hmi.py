...
class HMIWindow(Gtk.Window):
    ...
    def initModbus(self):
        ...
        self.modbusClient = ModbusClient('172.16.192.2', port=502)
    ...
    def __init__(self):
        Gtk.Window.__init__(self, title="CybatiWorks Bottle-filling factory - HMI - VirtuaPlant      ")
        ...
        # Main title label
        label = Gtk.Label()
        label.set_markup("<span weight='bold' size='x-large'>CybatiWorks Bottle-filling process status</span>")
    ...
    def update_status(self):
        ...
        try:
            ...
            if regs[1] == 1:
                self.bottlePositionValue.set_markup("<span weight='bold' foreground='black'>YES</span>")
            else:
                self.bottlePositionValue.set_markup("<span weight='bold' foreground='red'>NO</span>")

            if regs[0] == 1:
                self.levelHitValue.set_markup("<span weight='bold' foreground='black'>YES</span>")
            else:
                self.levelHitValue.set_markup("<span weight='bold' foreground='red'>NO</span>")

            if regs[2] == 1:
                self.motorStatusValue.set_markup("<span weight='bold' foreground='black'>ON</span>")
            else:
                self.motorStatusValue.set_markup("<span weight='bold' foreground='red'>OFF</span>")

            if regs[3] == 1:
                    self.nozzleStatusValue.set_markup("<span weight='bold' foreground='black'>OPEN</span>")
            else:
                self.nozzleStatusValue.set_markup("<span weight='bold' foreground='red'>CLOSED</span>")

            if regs[15] == 1:
                self.processStatusValue.set_markup("<span weight='bold' foreground='black'>RUNNING</span>")
            else:
                self.processStatusValue.set_markup("<span weight='bold' foreground='red'>STOPPED</span>")

            self.connectionStatusValue.set_markup("<span weight='bold' foreground='black'>ONLINE</span>")
