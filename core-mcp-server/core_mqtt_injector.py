#!/usr/bin/env python3
"""
CORE MQTT Injector - Injects sensor data INTO the CORE network

This script solves the problem of getting physical sensor data (from micro:bit via
WebSerial in browser) to flow as REAL network traffic inside CORE so it's visible
in Wireshark.

Architecture:
    Browser (WebSerial) -> REST API -> This Injector -> vcmd/docker exec -> CORE Network
...
Usage:
    # Start the injector (runs alongside web_ui.py)
    python3 core_mqtt_injector.py --session 1 --broker-node mqtt-broker --broker-ip 10.0.1.10

    # Or auto-detect from running CORE session
    python3 core_mqtt_injector.py --auto
"""
...
class COREMQTTInjector:
    """Injects MQTT messages into CORE network using vcmd/docker exec"""
    ...
    def _check_dependencies(self):
        ...
        if not self.is_docker and not os.path.exists(vcmd_path):
            print(f"[INJECTOR] Warning: vcmd socket not found at {vcmd_path}")
            print(f"[INJECTOR] Make sure CORE session {self.session_id} is running")
...
class InjectorHTTPHandler(BaseHTTPRequestHandler):
    """HTTP handler for receiving sensor data and injecting into CORE"""

    injector: COREMQTTInjector = None  # Set by server
    ...
    def do_POST(self):
        """Handle POST /inject - inject sensor data into CORE network"""
...
def main():
    parser = argparse.ArgumentParser(description="CORE MQTT Injector - Inject sensor data into CORE network")
    ...
    parser.add_argument("--port", type=int, default=8089, help="HTTP server port for receiving data")
    ...
    if args.auto:
        print("[INJECTOR] Auto-detecting CORE session...")
        session_id = find_core_session()
        if not session_id:
            print("[INJECTOR] ERROR: No CORE session found")
            sys.exit(1)
        print(f"[INJECTOR] Found session: {session_id}")

        broker_info = find_mqtt_broker_node(session_id)
        if broker_info:
            broker_node, broker_ip, is_docker = broker_info
            print(f"[INJECTOR] Found broker: {broker_node} @ {broker_ip}")

    if not session_id:
        print("[INJECTOR] ERROR: No session ID provided. Use --session or --auto")
        sys.exit(1)

    # Create injector
    injector = COREMQTTInjector(
    ...
    # Test mode
    if args.test:
        print("[INJECTOR] Testing connection...")
    ...
    if not injector.test_connection():
        print("[INJECTOR] Warning: Initial connection test failed")
        print("[INJECTOR] Make sure CORE session is running and mqtt-broker node has mosquitto_pub")

    # Set up HTTP server
    InjectorHTTPHandler.injector = injector
    server = HTTPServer(('0.0.0.0', args.port), InjectorHTTPHandler)

    print(f"[INJECTOR] HTTP server listening on port {args.port}")
    print(f"[INJECTOR] Send sensor data to: POST http://localhost:{args.port}/inject/{{sensor_id}}")
    print(f"[INJECTOR] Data will be published to MQTT topic: core/sensors/{{sensor_id}}/data")
    print(f"[INJECTOR] The MQTT packets will be VISIBLE IN WIRESHARK on the CORE network!")
